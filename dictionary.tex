...
% Per-face fallback chains: the main face is Latin-only either way, so
% the vendored Gentium supplies Greek, Cyrillic, superscript letters
% and symbols (e.g. the swung dash) in the matching face; DejaVu Serif
...
% Main face: Lexicon No1 (TEFF; commercial, installed system-wide,
% NOT vendored) when the build system has it, otherwise XCharter
% (Matthew Carter's Bitstream Charter extended by Michael Sharpe; SIL
% OFL, shipped with TeX Live). XCharter is the nearest open relative:
% sturdy and economical, and a sibling of the Charis SIL used for IPA.
% Defining \publicfonts before \input forces XCharter regardless, which
% scripts/publish-public.sh does so the distributed PDF never embeds
% Lexicon. \dictmainface names the face in use for the colophon.
% Gentium (fonts/) supplies Greek, superscript letters and symbols
% either face lacks; DejaVu stays as last resort.
\ifdefined\publicfonts
  \def\dictuselexicon{0}
\else
  \IfFontExistsTF{Lexicon No1 Roman A}
    {\def\dictuselexicon{1}}{\def\dictuselexicon{0}}
\fi
\ifnum\dictuselexicon=1
  \newcommand{\dictmainface}{Lexicon No1}
  \setmainfont{Lexicon No1 Roman A}[
    ItalicFont = {Lexicon No1 Italic A},
    BoldFont = {Lexicon No1 Roman D},
    BoldItalicFont = {Lexicon No1 Italic D},
    Ligatures = TeX,
...
\else
  \newcommand{\dictmainface}{XCharter}
...
\include{cover}
% Blank verso behind the half-title so the title page falls on a recto.
\null\thispagestyle{empty}\clearpage
\include{forepage}
% The two titlepage environments reset the page counter; realign it so
% the impressum (physical page 4) is folio iv and rectos carry odd
% roman folios throughout the front matter.
\setcounter{page}{4}
\include{copyright}
\include{thanks}
% Blank verso behind the acknowledgements so the preface opens on a recto.
\null\thispagestyle{empty}\clearpage
\include{foreword}
...
% Folio 1 must land on a recto so mirrored-margin parity matches
% physical parity. The page counter tracks physical pages here (see the
% realignment above); after \clearpage it holds the number of the NEXT
% page, so when that is even (a verso) emit one blank first. \twocolumn
% performs its own \clearpage; do not add \cleardoublepage (it would
% carry running heads onto the blank).
\clearpage
